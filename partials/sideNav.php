    ...
    <ul>
      ...
      while ($i <= 35) {
        ...
        if ($i == 35) {
          ...
          echo '
                          <a class="childprice"><button value=' . json_encode($priceJson) . ' class="pricetrigger" type="button" onclick="showProductAjax(this.value)">Rs.' . $i . '-</button></a>
                      </li>
                      ';
        } else {
          $priceJson['searchPara'] = 'partials' . DIRECTORY_SEPARATOR . 'productPrinter.php?searchstart=' . $i . '&searchend=' . ($i + 5) . '&location=priceRange';
          echo '
                          <a class="childprice"><button value=' . json_encode($priceJson) . ' class="pricetrigger" type="button" onclick="showProductAjax(this.value)">Rs.' . $i . '-Rs.' . ($i + 5) . '</button></a>
                      </li>
                      ';
        }
        $i = ($i + 5);
